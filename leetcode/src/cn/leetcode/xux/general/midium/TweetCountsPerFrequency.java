...
class TweetCounts {
    ...
    TreeMap<Integer, Map<String, Integer>> map;
    ...
    public void recordTweet(String tweetName, int time) {
        ...
        Map<String, Integer> currTimeRecord = map.get(time);
        currTimeRecord.put(tweetName, currTimeRecord.getOrDefault(tweetName, 0)+1);
    }

    public List<Integer> getTweetCountsPerFrequency(String freq, String tweetName, int startTime, int endTime) {
        List<Integer> ans = new ArrayList<>();
        int len = freq.equals("minute")?60:(freq.equals("hour")?3600:3600*24);
        for(int start=startTime;start<=endTime;start+=len) {
            int end = Math.min(start+len-1, endTime);
            int cnt = 0;
            Integer currTime = map.ceilingKey(start);
            while(currTime!=null&&currTime<=end) {
                Map<String, Integer> currRecord = map.get(currTime);
                cnt += currRecord.getOrDefault(tweetName, 0);
                currTime = map.ceilingKey(currTime+1);
            }
            ans.add(cnt);
        }
        return ans;
