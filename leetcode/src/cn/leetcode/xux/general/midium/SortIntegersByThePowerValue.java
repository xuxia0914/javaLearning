...
public class SortIntegersByThePowerValue {
    ...
    private int find(int[][] arr, int start, int end, int k) {
        ...
        int left = start;
        int right = end;
        int[] key = arr[start].clone();
        while(left<right) {
            while(left<right&&(arr[right][1]>key[1]||(arr[right][1]==key[1]&&arr[right][0]>=key[0]))) {
                right--;
            }
            while(left<right&&(arr[left][1]<key[1]||(arr[left][1]==key[1]&&arr[left][0]<=key[0]))) {
                left++;
            }
            if(left<right) {
                int[] tmp = arr[left];
        ...
        arr[right] = key;
        if(right<k-1) {
            return find(arr, right+1, end, k);
        }else if(right>k-1) {
            return find(arr, start, right-1, k);
        }else {
            return arr[right][0];
        }
    }
