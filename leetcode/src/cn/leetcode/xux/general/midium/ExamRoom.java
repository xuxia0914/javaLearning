...
public class ExamRoom {
    ...
    public int seat() {
        ...
        int maxDis = seats.first();
        int result = 0;
        int rightDis = n-1-seats.last();
        if(rightDis>maxDis) {
            maxDis = rightDis;
            result = n-1;
        }
        for(Integer leftSeat : seats) {
            Integer rightSeat = seats.ceiling(leftSeat+1);
            if(rightSeat!=null) {
                int currSeat = (leftSeat+rightSeat)/2;
                int currDis = currSeat - leftSeat;
                if(currDis>maxDis||(currDis==maxDis&&currSeat<result)) {
                    result = currSeat;
                    maxDis = currDis;
                }
            }
