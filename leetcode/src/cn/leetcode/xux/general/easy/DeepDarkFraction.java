...
public class DeepDarkFraction {
    ...
    public int[] fraction(int[] cont) {
        return fraction(cont, 0);
    }

    public int[] fraction(int[] cont, int start) {
        if(start==cont.length-1) {
            return new int[]{cont[start], 1};
        }
        int[] pre = fraction(cont, start+1);
        return new int[]{cont[start]*pre[0]+pre[1], pre[0]};
    }
