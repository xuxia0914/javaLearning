        ...
        for(int i=m-1;i>=0;i--) {
            for(int j=n-1;j>=0;j--) {
                if(i==m-1&&j==n-1) {
                    ...
                    dp[i][j] = Math.max(1, Math.max(1-dungeon[i][j], dp[i][j+1]-dungeon[i][j]));
                }else if(j==n-1) {
                    dp[i][j] = Math.max(1, Math.max(1-dungeon[i][j], dp[i+1][j]-dungeon[i][j]));
                }else {
                    dp[i][j] = Math.max(1, Math.max(1-dungeon[i][j],
