...
public class ExclusiveTimeOfFunctions {
    ...
    public static int[] solution(int n, String[] logs) {
        ...
        for(String log : logs) {
            ...
            if(sa[1].equals("start")) {
                if(!stack.isEmpty()) {
                    int[] curr = stack.peek();
                    result[curr[0]] += Integer.valueOf(sa[2])-curr[1];
                }
                stack.push(new int[]{Integer.valueOf(sa[0]), Integer.valueOf(sa[2])});
            }else {
                int[] curr = stack.pop();
                result[curr[0]] += Integer.valueOf(sa[2])-curr[1]+1;
                if(!stack.isEmpty()) {
                    int[] preCurr = stack.pop();
                    preCurr[1] = Integer.valueOf(sa[2]) + 1;
                    stack.push(preCurr);
                }
            }
        }
...
//        int[] result = solution(2, new String[]{"0:start:0", "1:start:2", "1:end:5", "0:end:6"});
        int[] result = solution(2, new String[]{"0:start:0","0:start:2","0:end:5","1:start:7","1:end:7","0:end:8"});
        for(int i : result) {
            System.out.print(i + " ");
        }
    }
