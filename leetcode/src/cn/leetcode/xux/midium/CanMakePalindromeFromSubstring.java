...
public class CanMakePalindromeFromSubstring {
    ...
    public List<Boolean> canMakePaliQueries(String s, int[][] queries) {
        ...
        for(int[] query : queries) {
            int single = 0;
            for(int i=0;i<26;i++) {
                single += (cnts[query[1]+1][i]-cnts[query[0]][i])%2;
            }
            ans.add(query[2]>=single/2);
        }
        return ans;
    }
    ...
    public List<Boolean> canMakePaliQueries1(String s, int[][] queries) {
        ...
        for(int[] query : queries) {
            ans.add(canMakePaliQueries1(s, query));
        }
        return ans;
    }

    public boolean canMakePaliQueries1(String s, int[] query) {
        int[] cnts = new int[26];
