...
public class PaintHouseIII {
    ...
    public int minCost(int[] houses, int[][] cost, int m, int n, int target) {
        // dp[i][j][k] 表示到第i个房子为止，形成j个街区且第i个房子的颜色为k的最小花费
            ...
            for(int j=1;j<=target;j++) {
                for(int k=1;k<=n;k++) {
                    if(dp[i][j][k]!=Integer.MAX_VALUE) {
                        if(houses[i+1]!=0) {
                            if(k==houses[i+1]) {
                                dp[i+1][j][houses[i+1]] = Math.min(dp[i+1][j][houses[i+1]], dp[i][j][houses[i+1]]);
                            }else if(j<target) {
                                dp[i+1][j+1][houses[i+1]] = Math.min(dp[i+1][j+1][houses[i+1]], dp[i][j][k]);
                            }
                        }else {
                            for(int color=1;color<=n;color++) {
                                if(color==k) {
                                    dp[i+1][j][color] = Math.min(dp[i+1][j][color], dp[i][j][k]+cost[i+1][color-1]);
                                }else if(j<target) {
                                    dp[i+1][j+1][color] = Math.min(dp[i+1][j+1][color], dp[i][j][k]+cost[i+1][color-1]);
                                }
                            }
                        }
                    }
