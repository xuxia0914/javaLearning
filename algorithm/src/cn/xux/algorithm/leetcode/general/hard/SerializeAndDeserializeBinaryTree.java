...
public class SerializeAndDeserializeBinaryTree {
    ...
    public String serialize(cn.xux.algorithm.common.TreeNode root) {
        StringBuilder sb = new StringBuilder();
        if(root==null) {
            return null;
        }
        Queue<cn.xux.algorithm.common.TreeNode> queue = new LinkedList<>();
        queue.offer(root);
        sb.append("[").append(root.val);
        while(!queue.isEmpty()) {
            cn.xux.algorithm.common.TreeNode curr = queue.poll();
            if(curr.left!=null) {
    ...
    public cn.xux.algorithm.common.TreeNode deserialize(String data) {
        ...
        cn.xux.algorithm.common.TreeNode root = new cn.xux.algorithm.common.TreeNode(Integer.valueOf(s[0]));
        Queue<cn.xux.algorithm.common.TreeNode> queue = new LinkedList<>();
        queue.offer(root);
        int len = s.length;
        int i = 1;
        while(i<len) {
            cn.xux.algorithm.common.TreeNode curr = queue.poll();
            if(!"null".equals(s[i])) {
                curr.left = new cn.xux.algorithm.common.TreeNode(Integer.valueOf(s[i]));
                queue.offer(curr.left);
            }
            i++;
            if(!"null".equals(s[i])) {
                curr.right = new cn.xux.algorithm.common.TreeNode(Integer.valueOf(s[i]));
                queue.offer(curr.right);
            }
            i++;
        }
