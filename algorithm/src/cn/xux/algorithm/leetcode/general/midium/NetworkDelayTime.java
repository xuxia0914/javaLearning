...
public class NetworkDelayTime {
    ...
    public int networkDelayTime(int[][] times, int N, int K) {
        ...
        for(int i=0;i<times.length;i++) {
            ...
            map.get(time[0]).add(i);
        }
        int[] expends = new int[N+1];
        Arrays.fill(expends, Integer.MAX_VALUE);
        expends[K] = 0;
        boolean[] visited = new boolean[N+1];
        networkDelayTime(times, map, K, visited, expends);
        int result = 0;
        for(int i=1;i<=N;i++) {
            if(expends[i]==Integer.MAX_VALUE) {
                return -1;
            }
            result = Math.max(result, expends[i]);
        }
        return result;
    }

    public void networkDelayTime(int[][] times, Map<Integer, List<Integer>> map, int curr, boolean[] visited, int[] expends) {
        if(visited[curr]) {
            return;
        }
        visited[curr] = true;
        ...
        for(int idx : map.get(curr)) {
            if(expends[curr]+times[idx][2]<expends[times[idx][1]]) {
                expends[times[idx][1]] = expends[curr]+times[idx][2];
                networkDelayTime(times, map, times[idx][1], visited, expends);
            }
        }
        visited[curr] = false;
    }
