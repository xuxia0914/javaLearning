...
/**
 * 2069. 模拟行走机器人 II
 * 给你一个在 XY 平面上的 width x height 的网格图，
 * 左下角 的格子为 (0, 0) ，右上角 的格子为 (width - 1, height - 1) 。
    ...
    class Robot {
        ...
        public int[] getPos() {
            if(curr<=w) {
                ...
                return new int[]{w, curr-w};
            }else if(curr<=(w<<1)+h) {
                return new int[]{(w<<1)+h-curr, h};
            }else {
                return new int[]{0, ((w+h)<<1)-curr};
            }
        }

        public String getDir() {
            if(curr==0||curr>(w<<1)+h) {
