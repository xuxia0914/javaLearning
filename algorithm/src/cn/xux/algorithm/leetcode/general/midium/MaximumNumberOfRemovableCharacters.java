 ...
 * 更正式的解释是，对于每个 0 <= i < k ，
 * 先标记出位于 s[removable[i]] 的字符，接着移除所有标记过的字符，
 * 然后检查 p 是否仍然是 s 的一个子序列。
 *
 ...
 * "ab" 是 "accb" 的一个子序列。
 * 如果移除下标 3、1 和 0 对应的字符后，"abcacb" 变成 "ccb" ，
 * 那么 "ab" 就不再是 s 的一个子序列。
 * 因此，最大的 k 是 2 。
 * 示例 2：
 *
...
public class MaximumNumberOfRemovableCharacters {
    ...
    public int maximumRemovals(String s, String p, int[] removable) {
        ...
        while(left<right) {
            ...
            boolean[] state = new boolean[len1];
            for(int i=0;i<mid;i++) {
    ...
    private boolean check(String s, String p, boolean[] state) {
        int len1 = s.length();
        int len2 = p.length();
        int idx = 0;
        for(int i=0;i<len1;i++) {
            if(!state[i]&&p.charAt(idx)==s.charAt(i)&&++idx==len2) {
                return true;
            }
        }
        return false;
