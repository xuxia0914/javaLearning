...
public class VideoStitching {
    ...
    public static void main(String[] args) {
        System.out.println(new VideoStitching().videoStitching(
//                new int[][]{{0,2},{4,6},{8,10},{1,9},{1,5},{5,9}}, 10
    ...
    public int videoStitching(int[][] clips, int T) {
        ...
        for(int i=0;i<n;i++) {
            if(clips[i][0]==0&&clips[i][1]>0) {
                int tmp = dfs(clips, T, i, new boolean[n]);
    ...
    public int dfs(int[][] clips, int T, int idx, boolean[] visited) {
        if(visited[idx]) {
            return -1;
        }
        if(clips[idx][1]>=T) {
        ...
        for(int i=0;i<clips.length;i++) {
            if(clips[i][0]>clips[idx][0]&&clips[i][0]<=clips[idx][1]
                    &&clips[i][1]>clips[idx][1]) {
                int nextResult = dfs(clips, T, i, visited);
                if(result==-1) {
                    if(nextResult!=-1) {
                        result = nextResult+1;
                    }
