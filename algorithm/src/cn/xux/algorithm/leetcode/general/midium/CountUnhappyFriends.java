...
public class CountUnhappyFriends {
    ...
    public int unhappyFriends(int n, int[][] preferences, int[][] pairs) {
        Set<Integer>[] mapping = new Set[n];
        for(int i=0;i<n;i++) {
            mapping[i] = new HashSet<>();
        }
        for(int[] pair : pairs) {
            for(int p : preferences[pair[0]]) {
                if(p!=pair[1]) {
                    mapping[pair[0]].add(p);
                }else {
                    break;
                }
            }
            for(int p : preferences[pair[1]]) {
                if(p!=pair[0]) {
                    mapping[pair[1]].add(p);
                }else {
                    break;
                }
            }
        ...
        int ans = 0;
        for(int[] pair : pairs) {
            for(int u : mapping[pair[0]]) {
                if(mapping[u].contains(pair[0])) {
                    ans++;
                    break;
                }
            }
            for(int u : mapping[pair[1]]) {
                if(mapping[u].contains(pair[1])) {
                    ans++;
                    break;
                }
            }
