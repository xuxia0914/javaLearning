...
/**
 * 2241. 设计一个 ATM 机器
 * 一个 ATM 机器，存有 5 种面值的钞票：
 * 20 ，50 ，100 ，200 和 500 美元。
 * 初始时，ATM 机是空的。用户可以用它存或者取任意数目的钱。
 *
 * 取款时，机器会优先取 较大 数额的钱。
 *
 * 比方说，你想取 $300 ，并且机器里有 2 张 $50 的钞票，
 * 1 张 $100 的钞票和1 张 $200 的钞票，
 * 那么机器会取出 $100 和 $200 的钞票。
 * 但是，如果你想取 $600 ，
 * 机器里有 3 张 $200 的钞票和1 张 $500 的钞票，
 * 那么取款请求会被拒绝，因为机器会先取出 $500 的钞票，
 * 然后无法取出剩余的 $100 。
...
public class DesignAnAtmMachine {
    ...
    class ATM {
        ...
        public int[] withdraw(int amount) {
            int[] ans = new int[5];
            for(int i=4;i>=0;i--) {
                ans[i] = (int)Math.min(cnt[i], (long)amount/money[i]);
