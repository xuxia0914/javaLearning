...
public class OperationsOnTree {
    ...
    class LockingTree {
        ...
        // parent[i]表示节点i的父节点
        int[] parent;
        // children[i]表示节点i的子节点列表
        List<Integer>[] children;
        // lockedUser[i]表示锁住节点i的用户，未被锁则为-1；
        int[] lockedUser;
        ...
        public LockingTree(int[] parent) {
            ...
            this.parent = parent;
            children = new List[n];
            lockedUser = new int[n];
            Arrays.fill(lockedUser, -1);
            for(int i=1;i<n;i++) {
        ...
        public boolean upgrade(int num, int user) {
            int curr = num;
            while(curr!=-1) {
                if(lockedUser[curr]!=-1) {
                    return false;
                }
                curr = parent[curr];
            }
            boolean hasLockNode = unlock(num);
            if(!hasLockNode) {
                return false;
            }
            lockedUser[num] = user;
