 ...
 * (m - 1, n - 1) 是最右下角的格子。
 * 给你一个整数数组 startPos ，startPos = [startrow, startcol]
 * 表示 初始 有一个 机器人 在格子 (startrow, startcol) 处。
 * 同时给你一个整数数组 homePos ，
 * homePos = [homerow, homecol] 表示机器人的 家 在格子 (homerow, homecol) 处。
 *
 * 机器人需要回家。每一步它可以往四个方向移动：上，下，左，右，同时机器人不能移出边界。
 * 每一步移动都有一定代价。再给你两个下标从 0 开始的额整数数组：
 * 长度为 m 的数组 rowCosts  和长度为 n 的数组 colCosts 。
 *
