 ...
 * 如果不存在下标为 k 的元素 nums[k] 属于不止一个子数组，
 * 就称这些子数组是 不相交 的。子数组指的是原数组中连续元素组成的一个序列。
 *
 * 示例 1：
 ...
 * 输出：false
 * 解释：选择子数组 [7,7,1,2,3,4,7,7] 和 [7,7,1,2,3,4,7,7] 是不正确的，
 * 因为它们不是不相交子数组。
...
public class FormArrayByConcatenatingSubarraysOfAnotherArray {
    ...
    public boolean canChoose(int[][] groups, int[] nums) {
        ...
        for(int[] group : groups) {
            while(nums.length-idx<group.length||!match(group, nums, idx)) {
                if(nums.length-idx<group.length) {
                    return false;
                }else {
                    idx++;
                }
    ...
    private boolean match(int[] group, int[] nums, int idx) {
