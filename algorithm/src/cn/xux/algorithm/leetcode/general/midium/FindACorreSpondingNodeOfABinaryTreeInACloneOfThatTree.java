 ...
 * 输出: 3
 * 解释: 上图画出了树 original 和 cloned。target 节点在树 original 中，
 * 用绿色标记。答案是树 cloned 中的黄颜色的节点（其他示例类似）。
 *
 * 示例 2:
 * 输入: tree = [7], target =  7
 * 输出: 7
...
public class FindACorreSpondingNodeOfABinaryTreeInACloneOfThatTree {
    ...
    public TreeNode dfs(TreeNode node1, TreeNode node2, TreeNode target) {
        if(node1==null) {
            return null;
        }
        if(node1==target) {
        ...
        TreeNode leftAns = dfs(node1.left, node2.left, target);
        if(leftAns==null) {
            return dfs(node1.right, node2.right, target);
        }
        return leftAns;
    }
