...
public class MaximumProfitOfOperatingACentennialWheel {
    ...
    public static void main(String[] args) {
        System.out.println(new MaximumProfitOfOperatingACentennialWheel()
                .minOperationsMaxProfit(new int[]{8, 3}, 5, 6));
    }

    public int minOperationsMaxProfit(
            int[] customers, int boardingCost, int runningCost) {
        ...
        int turn = 0;
        while(wait>0||turn<n) {
            int on = Math.min(4, wait+(turn<n?customers[turn]:0));
            onWheel += on;
            wait += (turn<n?customers[turn]:0)-on;
            profit = onWheel*boardingCost-(turn+1)*runningCost;
            if(profit>max) {
                max = profit;
                ans = turn+1;
            }
