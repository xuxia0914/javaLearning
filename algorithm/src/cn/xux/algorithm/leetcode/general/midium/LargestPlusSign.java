...
public class LargestPlusSign {
    ...
    public int orderOfLargestPlusSign(int N, int[][] mines) {
        ...
        for(int i=0;i<N;i++) {
            for(int j=0;j<N;j++) {
                if(matrix[i][j]==1) {
                    dp[i][j][0] = 1+(i>0?dp[i-1][j][0]:0);
                    dp[i][j][2] = 1+(j>0?dp[i][j-1][2]:0);
                }
            }
        }
        for(int i=N-1;i>=0;i--) {
            for(int j=N-1;j>=0;j--) {
                if(matrix[i][j]==1) {
                    dp[i][j][1] = 1+(i<N-1?dp[i+1][j][1]:0);
                    dp[i][j][3] = 1+(j<N-1?dp[i][j+1][3]:0);
                    result = Math.max(result, Math.min(Math.min(dp[i][j][0], dp[i][j][1]),Math.min(dp[i][j][2], dp[i][j][3])));
                }
            }
        }
        return result;
