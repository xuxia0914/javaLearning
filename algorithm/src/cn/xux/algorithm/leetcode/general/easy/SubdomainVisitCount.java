 ...
 * 说明: 例子中仅包含一个网站域名："discuss.leetcode.com"。
...
public class SubdomainVisitCount {
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for(String cpdomain : cpdomains) {
            ...
            map.put(strs[1], map.getOrDefault(strs[1], 0)+cnt);
            int dotIdx1 = strs[1].indexOf(".");
            if(dotIdx1!=-1) {
                String tmp = strs[1].substring(dotIdx1+1);
                map.put(tmp, map.getOrDefault(tmp, 0)+cnt);
            }
            int dotIdx2 = strs[1].indexOf(".", dotIdx1+1);
            if(dotIdx2!=-1) {
                String tmp = strs[1].substring(dotIdx2+1);
                map.put(tmp, map.getOrDefault(tmp, 0)+cnt);
            }
        }
